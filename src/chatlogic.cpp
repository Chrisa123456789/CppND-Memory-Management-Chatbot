...
ChatLogic::ChatLogic() { //CG changed

    //// STUDENT CODE (Task 3)
    ////
  
    //cout << "ChatLogic Constructor" << std::endl;
...
ChatLogic::~ChatLogic() { //CG changed
    ...
    ////
  	...
  	//cout << "ChatLogic Destructor" << std::endl;
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](std::unique_ptr<GraphNode>& node) { return node->GetID() == id; }); //CG changed
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        {
                            // get iterator on incoming and outgoing node via ID search //CG changed
    ...
    ChatBot aChatBot("../images/chatbot.png");
  
    //SetChatbotHandle(&aChatBot);  
    aChatBot.SetChatLogicHandle(this);
    aChatBot.SetRootNode(rootNode);
    rootNode->MoveChatbotHere(std::move(aChatBot));
